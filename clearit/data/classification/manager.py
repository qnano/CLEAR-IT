...
class ClassificationDataManager:
    """
    Build train/validation data loaders for classification heads.

    Modes:
    - Eager mode pre-extracts all multi-channel crops into a tensor.
    - Lazy mode crops on demand from full images with an LRU-backed image cache.

    Caches:
    - Eager mode stores `(crop_df, crops_tensor)` in memory.
    - Lazy mode stores a reusable `ClassificationCropDataset`.
    """
    ...
    def get_dataloader(dataset_name,
        ...
        if lazy_mode:
            # Lazy mode: on-the-fly cropping over a reusable base dataset.
            ...
            # Split into train/val subsets that share the same LRU image cache.
            if test_size > 0:
        ...
        # Eager mode: pre-extract all crops before constructing the datasets.
